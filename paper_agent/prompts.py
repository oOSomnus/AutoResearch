...
**第三层：现有不足（研究缺口）**
- 说明现有方法或技术存在的局限性：
  - 列出具体的技术缺陷（如计算复杂度高、内存占用大、可解释性差等）
  - 说明这些局限性在实际应用中的表现
- 这些不足带来了什么具体问题：
  - 举例说明：例如，现有方法在处理X情况时会出现Y错误
  - 引用数据：如"现有方法准确率仅70%，无法满足工业需求"
- 用具体的数据或实例说明（如果论文中有）：
  - 引用论文中对比实验的数据（表2显示现有方法在数据集Z上的性能）
  - 引用相关工作的具体不足之处（如方法A不支持功能X，方法B在Y条件下失效）
- 如果论文没有数据，基于行业知识补充现实例子
...
**第三层：具体实现（技术细节）**
- **对于算法**：必须逐步分解，列出每个步骤的输入、处理、输出
  - 提供完整的算法执行流程（如：步骤1做X，步骤2检查Y，步骤3返回Z）
  - 说明算法的初始化过程和终止条件
  - 如果是复杂算法（如调度算法），列出所有关键步骤及其顺序
- **对于系统架构**：详细说明每个组件的功能、数据流、交互方式
  - 说明组件之间的接口定义和调用关系
  - 描述数据在各组件间的流转过程
- **引用具体的公式、代码片段、伪代码**：
  - 引用论文中包含的具体公式，并解释每个符号的含义
  - 如果论文提供伪代码，引用并解释关键行
- **说明关键参数的具体值及其作用**：
  - 列出所有重要参数及其默认值或推荐值
  - 解释每个参数为什么选择这个值
...
**第三层：具体数据（详细结果）**
- 包含具体的定量数据：
  - 关键性能指标：准确率、F1分数、召回率、精确率、AUC等的具体数值
  - 对比数据：比基线方法提升X%的具体计算过程和结果
  - 具体数字：例如"在X数据集上达到Y%的准确率，在Z数据集上达到W%的F1分数"
- 引用具体的表或图中的数据：
  - "表2显示我们的方法在数据集A上达到89.5%的准确率，比基线方法B提升12.3%"
  - "图3展示了随着训练轮数增加，准确率从60%逐步提升到92%的趋势"
- 说明实验条件的细节：
  - 列出每个实验的数据集规模、特征维度、样本数量
  - 说明评估指标的详细定义和计算公式
  - 说明统计显著性检验的结果（如p值、置信区间）
- 对于消融实验，列出每个配置的具体性能数据
...
**第三层：具体设置（技术细节）**
- 详细说明实验设置的关键参数和条件：
  - 列出所有超参数及其具体值（如学习率、batch size、迭代次数等）
  - 说明每个参数的作用和选择理由
- 解释实验流程的详细步骤：
  - 步骤1：数据预处理，列出具体操作（归一化、截断、增强等）
  - 步骤2：模型训练，列出训练过程细节（优化器、学习率调度策略）
  - 步骤3：评估，列出评估指标和计算方法
- 说明实验环境的配置（硬件、软件等）：
  - GPU型号、内存大小、训练时间
  - 框架版本、依赖库版本
...
- 列出论文中提及的几个重要的相关工作或方法：
  - 每个工作都要说明：作者、发表年份、核心思想
  - 引用具体的工作名称（如"方法X (Author et al., 2020)"）
- 说明这些相关工作的主要贡献和特点：
  - 列出每个方法的关键技术创新点
  - 说明它们解决的核心问题
- 指出这些工作的不足或局限性：
  - 具体列出每个方法的缺陷（如计算复杂度高、适用范围受限、精度不足等）
  - 引用数据支持这些局限性的说明（如"方法X在数据集Y上的准确率仅为60%"）
- 说明当前研究如何改进或超越这些工作：
  - 列出具体的技术改进点
  - 说明这些改进如何解决了已有方法的局限性
...
- 详细分析每个局限性的原因：
  - 从技术角度分析为什么会出现这个局限性
  - 说明是算法设计问题、数据问题还是实验设置问题
- 说明这些局限性可能在什么情况下出现问题：
  - 列出具体的触发条件（如"当输入规模超过10K时，性能显著下降"）
  - 说明边界条件和极端情况的表现
- 引用论文中作者自己讨论的局限性：
  - 引用具体章节或段落（如"作者在第5.3节指出..."）
  - 列出作者提到的具体限制因素（计算资源、数据质量、适用场景等）
- 如果论文中缺少某些限制性讨论，基于你的分析补充合理的局限性分析
...
- Explain limitations of existing methods or technologies:
  - List specific technical defects (e.g., high computational complexity, large memory usage, poor interpretability, etc.)
  - Explain how these limitations manifest in practical applications
- What specific problems do these limitations create:
  - Provide examples: e.g., existing methods produce error Y when handling situation X
  - Reference data: "Existing methods achieve only 70% accuracy, insufficient for industrial needs"
- Use specific data or examples if present in the paper:
  - Reference comparison experiment data from the paper (Table 2 shows performance of existing methods on dataset Z)
  - Reference specific shortcomings of related work (e.g., Method A doesn't support feature X, Method B fails under condition Y)
- If paper lacks data, supplement with real-world examples based on industry knowledge
...
- **For algorithms**: Must decompose step-by-step, listing input, processing, and output for each step
  - Provide complete algorithm execution flow (e.g., Step 1 does X, Step 2 checks Y, Step 3 returns Z)
  - Explain algorithm initialization process and termination conditions
  - For complex algorithms (e.g., scheduling algorithms), list all key steps and their sequence
- **For system architecture**: Detail each component's function, data flow, and interaction methods
  - Explain interface definitions and call relationships between components
  - Describe data flow process across components
- **Reference specific formulas, code snippets, or pseudocode**:
  - Reference specific formulas from the paper and explain meaning of each symbol
  - If paper provides pseudocode, reference and explain key lines
- **Explain specific values and roles of key parameters**:
  - List all important parameters with their default or recommended values
  - Explain why each parameter value was chosen
- **Reference specific sections/figures from the paper to support descriptions**:
  - Explicitly reference: "As stated in Section 3.2", "Figure 4 shows..."
  - Reference specific data points: "Table 2 shows accuracy of 89.5%"
- **For complex mechanisms (e.g., scheduling, optimization)**:
  - Provide complete decision flow or state transition diagrams
  - Explain handling of boundary conditions and special cases
...
- Include specific quantitative data:
  - Key performance metrics: specific values for accuracy, F1 score, recall, precision, AUC, etc.
  - Comparison data: specific calculation process and results for X% improvement over baseline
  - Specific numbers: e.g., "achieved Y% accuracy on X dataset, W% F1 score on Z dataset"
- Reference specific data from tables or figures:
  - "Table 2 shows our method achieves 89.5% accuracy on dataset A, 12.3% improvement over baseline method B"
  - "Figure 3 demonstrates the trend of accuracy gradually improving from 60% to 92% as training epochs increase"
- Explain details of experimental conditions:
  - List dataset size, feature dimensions, sample count for each experiment
  - Explain detailed definitions and calculation formulas for evaluation metrics
  - Explain statistical significance test results (e.g., p-values, confidence intervals)
- For ablation studies, list specific performance data for each configuration
...
- Detail key parameters and conditions of experimental settings:
  - List all hyperparameters with their specific values (learning rate, batch size, epochs, etc.)
  - Explain the role and selection rationale for each parameter
- Explain detailed steps of experimental procedure:
  - Step 1: Data preprocessing, list specific operations (normalization, truncation, augmentation, etc.)
  - Step 2: Model training, list training process details (optimizer, learning rate scheduling strategy)
  - Step 3: Evaluation, list evaluation metrics and calculation methods
- Describe experimental environment configuration (hardware, software, etc.):
  - GPU model, memory size, training time
  - Framework version, dependency library versions
- Reference specific parameter values from the paper:
  - "As stated in Section 4, learning rate is set to 0.001"
  - "Table 1 lists all hyperparameter configurations"
...
- List several important related works or methods mentioned in the paper:
  - For each work, specify: authors, publication year, core idea
  - Reference specific work names (e.g., "Method X (Author et al., 2020)")
- Explain main contributions and characteristics of these related works:
  - List key technical innovations of each method
  - Explain the core problems they solve
- Point out shortcomings or limitations of these works:
  - Specifically list defects of each method (e.g., high computational complexity, limited applicability, insufficient accuracy, etc.)
  - Reference data to support limitation explanations (e.g., "Method X achieves only 60% accuracy on dataset Y")
- Explain how current research improves or goes beyond these works:
  - List specific technical improvements
  - Explain how these improvements address limitations of existing methods
...
- Analyze reasons for each limitation in detail:
  - Analyze from technical perspective why this limitation occurs
  - Explain whether it's an algorithm design issue, data issue, or experimental setup issue
- Explain under what circumstances these limitations might cause problems:
  - List specific triggering conditions (e.g., "When input size exceeds 10K, performance significantly degrades")
  - Explain behavior in boundary conditions and extreme cases
- Reference limitations discussed by the authors themselves in the paper:
  - Reference specific sections or paragraphs (e.g., "The authors point out in Section 5.3 that...")
  - List specific limiting factors mentioned by authors (computational resources, data quality, applicability scenarios, etc.)
- If paper lacks certain limitation discussions, supplement with reasonable limitation analysis based on your own analysis
...
DETAIL_MODIFIERS = {
    ...
    "standard": {
        "background": "Include Tiers 1-3 (Layman Understanding + Key Issues + Existing Gaps), 3-4 paragraphs total. For Tier 3, include specific data or examples from the paper.",
        "innovation": "Include all four tiers with balanced depth, 5-7 paragraphs total. This is the core of the report. For Tier 3 (Specific Implementation), include step-by-step algorithm breakdown, specific parameter values, and references to formulas/figures/sections.",
        "results": "Include Tiers 1-3 (Layman Understanding + Key Findings + Specific Data), 3-4 paragraphs total. For Tier 3, include specific quantitative data with exact numbers, references to tables/figures, and experimental condition details.",
        "methodology": "Include Tiers 1-3 (Layman Understanding + Key Components + Specific Settings), 2-3 paragraphs total. For Tier 3, include specific parameter values with reasons, detailed experimental procedure steps, and hardware/software configuration.",
        "related_work": "Include Tiers 1-3 (Layman Understanding + Core Lineage + Specific Works), 2-3 paragraphs total. For Tier 3, list specific works with authors/years, explain their contributions, and point out limitations with supporting data.",
        "limitations": "Include Tiers 1-3 (Layman Understanding + Key Limitations + Specific Analysis), 2-3 paragraphs total. For Tier 3, analyze reasons for limitations, triggering conditions, and reference author's own discussion.",
    },
    "detailed": {
        "background": "Include all four tiers with comprehensive detail, 4-6 paragraphs total. For Tier 3, provide detailed analysis of existing gaps with specific data and real-world examples.",
        "innovation": "Provide extensive detail across all four tiers, 8-12 paragraphs total. Emphasize Tier 3 (Technical Details) and Tier 4 (Deep Insights). For Tier 3, include complete step-by-step algorithm execution flow, all parameter values with reasoning, pseudocode when available, and comprehensive references to formulas/figures/sections.",
        "results": "Include all four tiers with comprehensive detail, 5-7 paragraphs total. Emphasize Tier 3 (Specific Data) with detailed quantitative analysis. Include all metric values with exact numbers, comprehensive table/figure references, statistical significance results, and ablation study details.",
        "methodology": "Include all four tiers with comprehensive detail, 4-5 paragraphs total. Emphasize Tier 3 (Specific Settings). Include all hyperparameters with values and reasons, complete experimental procedure with numbered steps, detailed environment specifications, and configuration justifications.",
        "related_work": "Include all four tiers with comprehensive detail, 4-5 paragraphs total. Emphasize Tier 4 (Field Insights). For Tier 3, provide detailed analysis of each related work including specific innovations, exact limitations with quantitative comparisons, and technical improvements.",
        "limitations": "Include all four tiers with comprehensive detail, 3-4 paragraphs total. Emphasize Tier 4 (Future Directions). For Tier 3, provide deep technical analysis of limitation causes, specific failure scenarios, and detailed reference to author discussions.",
    },
}
